...
float calculateCost(float** MatrizVertice, int numVertices, int* caminho) {
    int custo = 0;
...
float** geraMatrizAdjacenciaBYTE(FILE *arq,char *Linha,char *result, int numVertices){
    float **MatrizVertice;
    int i,j,ultimoCaracter;
    float pri,seg;

    MatrizVertice = (float**) malloc(numVertices*sizeof(float*));
    for(i = 0; i < numVertices; i++){
//        printf("%d\n",i);
        MatrizVertice[i] = (float*) malloc(numVertices*sizeof(float));
...
*/
void tspMstAprox(float** MatrizVertice,int numVertice, int* caminho, int inicio)
{
 ...
 while(!pilha_vazia(P))
 {
  ...
  {
   if(MST->parent[i] == v)/*itera por toda a lista de pais gerada pela MST e adciona na pilha os vertices cujo pai eh "v". É como se estivessemos realizando uma DFS na arvore geradora de menor custo obtida pelo algoritmo de Prim. Sempre que chegamos a um "nó folha", ao inves de subir na arvore (o problema nao permite que visitemos um vertice mais de uma vez), pulamos para o proximo vertice ainda nao explorado que se encontra no topo da pilha. Em seguida descemos na árvore geradora partindo desse ponto. O algoritmo se repete até que a pilha esteja vazia. Isso garante que todos os vertices serão colocados no  caminho. Note que, ao contrario da DFS padrao, nao precisamos manter um vetor de marcacao, já que na arvore geradora cada vertice so pode ser alcancado por meio de um outro unico vertice.*/
...
void buildPath(float** MatrizVertice,int numVertice, int* caminho, int inicio) {
    ...
    for(int i = 0; i < numVertice; i++) {
        float valor_referencia = FLT_MAX;/*Inicialmente todos os vertices sao tidos como estando no infinito. Note que precisamos visitar todas as cidades, daí o loop tem de ser a quantidade de vertices existentes do grafo*/
...
void buildRandomPath(float** MatrizVertice,int numVertice, int* caminho, int inicio) {
    int *inseridos = malloc(numVertice * sizeof(int));
